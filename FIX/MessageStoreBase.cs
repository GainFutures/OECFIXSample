...
namespace OEC.FIX.Sample.FIX
{
    abstract class MessageStoreBase : IMessageStore
    {
        ...
        {
            SenderSeqNumChanged?.Invoke(_sessionID, next);
        }

        protected void RaiseTargetSeqNumChanged(int next)
        {
            TargetSeqNumChanged?.Invoke(_sessionID, next);
        }
        ...
        protected static DateTime CalculateCreationTime(DateTime currentUtc, TimeSpan sessionStartTimeUtc)
        {
            ...
            {
                throw new ArgumentException("DateTimeKind is not UTC.", nameof(currentUtc));
            }
        ...
        private void SaveSessionParams()
        {
            ...
            SaveSessionParams(_sessionID, sessionParams);
        }
